...
spc_tbl_ [32,853 × 7] (S3: spec_tbl_df/tbl_df/tbl/data.frame)
 ...
 $ region     : chr [1:32853] "Michigan (MI)" "Pennsylvania (PA)" "Ohio (OH)" "Pennsylvania (PA)" ...
...
ggplot(filtered_data, aes(x = values, y = species, fill = region)) +
  geom_boxplot(outlier.shape = NA) +  # Use outlier.shape = NA instead of outliers = FALSE
